...
from tear_sheets.models import TearSheet, TearSheetDetail


def return_price_records_by_rule_type(tear_sheet_pk):
    tear_sheet = TearSheet.objects.get(pk=tear_sheet_pk)
...
def return_details_by_title(tear_sheet_pk):
    details = TearSheetDetail.objects.filter(tear_sheet__pk=tear_sheet_pk)
    names = []
    for x in details:
