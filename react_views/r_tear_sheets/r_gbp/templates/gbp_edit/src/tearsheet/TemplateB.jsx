...
function TableRow({sdata, price_record, index}) {
  ...
  }, [rule_type, display_one, display_two, list, listNoVat, trade, tradeNoVat])
  ...
  return(
		<>
			<tr key={index} className="hover:bg-gray-50 text-gray-400 text-left">
				...
				edit_two ?
					<td style={{'width':`${sdata.col_3}px`}}><input onChange={ (event) => onChangeHandler(event,setTwo)} className={SETTINGS.input_classes} value={display_two}></input></td>
					:<td style={{'width':`${sdata.col_3}px`}} onClick={ () => onClickHandler(setEditTwo) } >{display_two}</td>
				}{
				edit_list ?
					<td style={{'width':`${sdata.col_4}px`}}><input onChange={ (event) => onChangeHandler(event,setList)} className={SETTINGS.input_classes} value={list}></input></td>
					:<td style={{'font-weight': 'bold', 'width':`${sdata.col_4}px`}}  onClick={() => { onClickHandler(setEditList) }} >£{list}</td>
				}
				{
				edit_listNoVat ?
					<td style={{'width':`${sdata.col_5}px`}}><input onChange={ (event) => onChangeHandler(event,setListNoVat)} className={SETTINGS.input_classes} value={listNoVat}></input></td>
					:<td style={{'width':`${sdata.col_5}px`}} onClick={ () => { onClickHandler(setEditListNoVat)} } >£{listNoVat}</td>
				}
				{
				edit_trade ?
					<td style={{'width':`${sdata.col_6}px`}}><input onChange={ (event) => onChangeHandler(event,setTrade)} className={SETTINGS.input_classes} value={trade}></input></td>
					:<td style={{'font-weight': 'bold', 'width':`${sdata.col_6}px`}} onClick={ () => { onClickHandler(setEditTrade)} } >£{trade}</td>
				}
				{
				edit_tradeNoVat ?
					<td style={{'width':`${sdata.col_7}px`}}><input onChange={ (event) => onChangeHandler(event,setEditTradeNoVat)} className={SETTINGS.input_classes} value={tradeNoVat}></input></td>
					:<td style={{'width':`${sdata.col_7}px`}} onClick={ () => { onClickHandler(setEditTradeNoVat)} } >£{tradeNoVat}</td>
				}
			</tr>
		</>
	)
