...
function App() {
  ...
  const print_url = CONTEXT.print_url
  const print_gbp_url = CONTEXT.print_gbp_url

  return (
    <div>

      <div className="grid grid-cols-2">
        <div style={{'text-align': 'right'}} className="text-end select gbp w-100">
            <label for="GBP"></label>
            <select onChange={ () => { setGbp( (event.target.value == 1) )} } name="GBP">
              <option value='0'>$ USD</option>
              <option value='1'>£ GBP</option>
            </select>
            <a style={{display:'block'}} href={ gbp ? print_gbp_url : print_url}>🖨️ </a>
        </div>
        <div className="empty">
        </div>
      </div>
